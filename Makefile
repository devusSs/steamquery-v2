...
build:
	...
	@echo "Cross-compiling app for Windows (AMD64), Linux (AMD64 & ARM64) & MacOS (ARM64)..."
	@go mod tidy
	...
	@GOOS=linux GOARCH=arm64 go build -v -trimpath -ldflags="-s -w -X 'github.com/devusSs/steamquery-v2/updater.BuildVersion=$(BUILD_VERSION)' -X 'github.com/devusSs/steamquery-v2/updater.BuildDate=${shell date}' -X 'github.com/devusSs/steamquery-v2/updater.BuildMode=$(BUILD_MODE)' -X 'github.com/devusSs/steamquery-v2/updater.BuildGitCommit=${shell git rev-parse HEAD}'" -o release/steamquery_linux_arm64/ ./...
